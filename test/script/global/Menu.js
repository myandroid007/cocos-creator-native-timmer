...
import SceneNavigator, {SceneRoute} from '../../../main/script/commons/scene/SceneNavigator';
...
cc.Class({
    ...
    onLoad: function () {
        ...
        this.btnBackList.node.active = false;
        this.loadInstruction(this.currentSceneUrl);
        this.node.zIndex = 999;

        SceneNavigator.initRoute({ sceneName: 'TestList.fire', onLaunched: this.onLoadSceneFinish.bind(this) });
    ...
    backToLastScene: function () {
        ...
        let lastSceneRoute = SceneNavigator.peekLast();
        if (lastSceneRoute.sceneName === 'TestList.fire') {
            this._isLoadingScene = false;
            this.backToList();
        } else {
            this.currentSceneUrl = lastSceneRoute.sceneName;
            SceneNavigator.pop({onLaunched: () => { this._isLoadingScene = false }});
        // SceneNavigator.pop();
        }
    },
    ...
    backToList: function () {
        ...
        SceneNavigator.popToTop({onLaunched: this.onLoadSceneFinish.bind(this)});
    },
    ...
    loadScene: function (url) {
        ...
        SceneNavigator.push({ sceneName: url, onLaunched: this.onLoadSceneFinish.bind(this) });
        if (typeof cocosAnalytics !== 'undefined') {
            //Cocos Analytics service, to learn more please visit:
            // http://analytics.qudao.info/
    ...
    onLoadSceneFinish: function () {
        ...
        if (this.isMenu && this.contentPos) {
            ...
            this.btnBackList.node.active = true;
            this.testList.node.active = false;
        }
        this._isLoadingScene = false;
    },
    ...
    loadInstruction: function (url) {
        ...
        cc.loader.loadRes('test/readme/' + fileName, function (err, txt) {
            if (err) {
