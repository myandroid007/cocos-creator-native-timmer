const TEST_CASE_BASE_DIR = 'db://assets/test/case/';
    ...
    initList () {
        ...
        if (scenes) {
            ...
            for (i = 0; i < scenes.length; ++i) {
                ...
                // let dirname = cc.path.dirname(url).replace('db://assets/test/case/', '');
                ...
                // 只取测试用例中的场景
                let testCaseBaseDir = cc.path.dirname(url);
                if (!testCaseBaseDir.startsWith(TEST_CASE_BASE_DIR)) {
                    continue;
                }
                let dirname = testCaseBaseDir.replace(TEST_CASE_BASE_DIR, '');
                let scenename = cc.path.basename(url, '.fire'); 
        ...
        this.initItemCount = Math.min(this.initItemCount, this.sceneList.length); 
